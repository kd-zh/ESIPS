...
def compo_detection(input_img_path, output_root, uied_params, name,
                    classifier=None, show=False, wait_key=None):
    ...
    binary = pre.binarization(org, grad_min=int(uied_params['min-grad']), show=show, wait_key=wait_key)
    
    # *** Step 2 *** element detection
    det.rm_line(binary, show=show, wait_key=None)
    # det.rm_line_v_h(binary, show=show)
    uicompos = det.component_detection(binary, min_obj_area=int(uied_params['min-ele-area']))
    # draw.draw_bounding_box(org, uicompos, show=show, name='components', wait_key=wait_key)
    ...
    # draw.draw_bounding_box(org, uicompos, show=show, name='merged', wait_key=wait_key)
    ...
    Compo.compos_update(uicompos, org.shape)
    draw.draw_bounding_box(org, uicompos, show=show, name='merged compo', write_path=pjoin(ip_root, name + '_uied.jpg'))
    ...
    print("[Compo Detection Completed in %.3f s] %s" % (time.process_time() - start, input_img_path))
    # if show:
